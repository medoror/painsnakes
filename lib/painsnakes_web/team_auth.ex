defmodule PainsnakesWeb.TeamAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_painsnakes_web_team_remember_me"
  ...
  def log_out_team(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      PainsnakesWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_team:
      ...
      defmodule PainsnakesWeb.PageLive do
        use PainsnakesWeb, :live_view

        on_mount {PainsnakesWeb.TeamAuth, :mount_current_team}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{PainsnakesWeb.TeamAuth, :ensure_authenticated}] do
        live "/profile", ProfileLive, :index
      end
  """
  def on_mount(:mount_current_team, _params, session, socket) do
    {:cont, mount_current_team(socket, session)}
  end

  def on_mount(:ensure_authenticated, _params, session, socket) do
    socket = mount_current_team(socket, session)
  ...
  defp mount_current_team(socket, session) do
