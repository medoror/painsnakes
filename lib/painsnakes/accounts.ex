defmodule Painsnakes.Accounts do
  @moduledoc """
  ...
  alias Painsnakes.Accounts.{Team, TeamToken, TeamNotifier}
  ...
  def get_team_by_name_and_password(name, password)
    ...
    team = Repo.get_by(Team, team_name: name)
    if Team.valid_password?(team, password), do: team
  end

  def get_team_by_team_name(team_name) when is_binary(team_name) do
    Repo.get_by(Team, team_name: team_name)
  end

  def team_name_taken?(team_name) when is_binary(team_name) do
    Repo.exists?(from t in Team, where: t.team_name == ^team_name)
  end

  def update_team_name(team, password, team_name) do
    team
    |> Team.team_name_changeset(%{team_name: team_name})
  ...
  def change_team_name(team, attrs \\ %{}) do
    Team.team_name_changeset(team, attrs)
  end
