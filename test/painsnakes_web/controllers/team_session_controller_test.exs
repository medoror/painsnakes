defmodule PainsnakesWeb.TeamSessionControllerTest do
  ...
  describe "POST /teams/log_in" do
    test "logs the team in", %{conn: conn, team: team} do
      conn =
        post(conn, ~p"/teams/log_in", %{
          "team" => %{"team_name" => team.team_name, "password" => valid_team_password()}
        })
    ...
    test "logs the team in with remember me", %{conn: conn, team: team} do
      ...
      assert conn.resp_cookies["_painsnakes_web_team_remember_me"]
      assert redirected_to(conn) == ~p"/"
    end
    ...
    test "redirects to login page with invalid credentials", %{conn: conn} do
      conn =
        post(conn, ~p"/teams/log_in", %{
          "team" => %{"team_name" => "bad_team_name", "password" => "invalid_password"}
        })
