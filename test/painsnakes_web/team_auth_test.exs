defmodule PainsnakesWeb.TeamAuthTest do
  use PainsnakesWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Painsnakes.Accounts
  alias PainsnakesWeb.TeamAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, PainsnakesWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_team/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      PainsnakesWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :redirect_if_team_is_authenticated" do
    ...
    test "doesn't redirect if there is no authenticated team", %{conn: conn} do
      session = conn |> get_session()
